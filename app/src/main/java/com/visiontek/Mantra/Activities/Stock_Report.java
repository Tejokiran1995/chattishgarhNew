...
public class Stock_Report extends AppCompatActivity implements PrinterCallBack {
    ...
    private Stock_Report mActivity;
            ...
            public void onClick(View v) {
               if (flag_print==1) {
                   ...
                   for (int i = 0; i < comm.size(); i++) {
                       app = comm.get(i) + "  " + scheme.get(i) + "  " + stock.get(i) + "  " + issue.get(i)
                               + "   " + cb.get(i) + "\n";
                       add.append(app);
                   }
...
// Here is if not while, indicating that I only want to support a device
    ...
    private void hitURL(String xmlformat) {
        pd = ProgressDialog.show(context, context.getResources().getString(R.string.Dealer), context.getResources().getString(R.string.Fetching_Dealers), true, false);
        XML_Parsing request = new XML_Parsing(context, xmlformat, 6);
        request.setOnResultListener(new XML_Parsing.OnResultListener() {
            ...
            public void onCompleted(String isError, String msg, String ref, String flow) {
                ...
                if (!isError.equals("00")) {
                    ...
                    flag_print = 1;
                    cb = databaseHelper.get_SD(0);
                    comm = databaseHelper.get_SD(1);
                    scheme = databaseHelper.get_SD(4);
                    stock = databaseHelper.get_SD(3);
                    issue = databaseHelper.get_SD(2);

                    data = new ArrayList<>();
                    for (int i = 0; i < comm.size(); i++) {
                        data.add(new DataModel3(comm.get(i), scheme.get(i), stock.get(i), issue.get(i), cb.get(i)));
                    }
